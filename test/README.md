...
### 🎓 Thông tin Đề tài
| Thuộc học phần | Cấu trúc dữ liệu và Giải thuật |
| :--- | :--- |
| Lớp | Kỹ sư Tài năng |
| **Sinh viên** | Nguyễn Anh Quân và Trần Tấn Phát |
...
### Tóm tắt Hệ thống
Dự án **SCAD-TSA** triển khai một pipeline hiệu năng cao (C++/Python) để xử lý và phân tích các tập dữ liệu chuỗi lớn (ví dụ: log files, URL, text). Hệ thống sử dụng cấu trúc dữ liệu **Trie** để nén dữ liệu và áp dụng các đặc trưng thống kê như **Tần suất**, **Độ dài** và **Entropy Cục bộ** tại mỗi nút để xác định các chuỗi có cấu trúc hoặc hành vi bất thường (**Anomalies**).

Phương pháp phát hiện bất thường sử dụng kỹ thuật **Phần vị (Percentile)**, đảm bảo tính **Vững chắc (Robustness)** trước sự lệch (skewness) của phân phối tần suất chuỗi.
...
| :--- | :--- | :--- | :--- |
| **Bất thường Tần suất** | Tần suất (Count) | **Thấp hơn P5** | Chuỗi hiếm (Rare String), xuất hiện không đủ thường xuyên để được coi là mẫu chuẩn. |
| **Bất thường Độ dài** | Tần suất độ dài (Length frequency) | **Thấp hơn P5** | Chuỗi có độ dài hiếm, ít gặp trong phân phối tần suất độ dài chung. |
...
| :--- | :--- | :--- | :--- |
| **`main_pipeline`** | **Điều phối** các module, xử lý flags (`--visual-x`, `--regex=`). | `<input_file>`, `<output_dir>` | - |
| **`bin/preprocess`** | Chuẩn hóa, làm sạch chuỗi. | `<input_file>` | `cleaned_input_text.txt` |
| **`bin/analyze`** | Xây dựng Trie, tính toán Phần vị, đánh dấu bất thường, xuất báo cáo. | `cleaned_input_text.txt` | `overall_report.txt`, `all_entries.csv`, **{3 file CSV Anomaly}**, **{Các file JSON cho Visualize}** |
...
* `all_entries.csv`: Bảng thống kê đầy đủ (Count, Length, Entropy) của TẤT CẢ các chuỗi duy nhất.
...
### 🏗️ Hướng dẫn Biên dịch (Dành cho Local và Colab)

```bash
# Giả định: Thư viện nlohmann/json.hpp nằm trong thư mục 'include/'

# Bước 1: Tạo thư mục chứa các executable
mkdir -p bin

# Bước 2: Biên dịch các module C++
# (Cần liên kết với thư viện nlohmann/json)
# Biên dịch Preprocess
g++ -std=c++17 -I./include src/preprocess_module.cpp -o bin/preprocess

# Biên dịch Analyze (module cốt lõi)
g++ -std=c++17 -I./include src/analyze_module.cpp -o bin/analyze

# Biên dịch Visualize Wrapper
g++ -std=c++17 -I./include src/visualize_module.cpp -o bin/visualize

# Biên dịch Chương trình Điều phối Chính (Main Pipeline)
